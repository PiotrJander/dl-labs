...
def setup():
    ...
    with tf.name_scope('input'):
        ...
        with tf.Session() as sess:
            ...
            # shape = sess.run(tf.shape(batch_images), feed_dict={batch_start: 19})
            # print(shape)

            # summary = sess.run(image_summaries('first'), feed_dict={batch_start: 0})
...
def upconv(features, in_channels, out_channels, name='upconv'):
    with tf.variable_scope(name):
        stride = 2
        weights = tf.get_variable("weights", [3, 3, out_channels, in_channels],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
        return tf.nn.conv2d_transpose(features, weights, strides=[1, stride, stride, 1], padding='SAME')


def bn_upconv_relu(features, in_channels, out_channels, name='bn_upconv_relu'):
    with tf.variable_scope(name):
        scale = tf.get_variable("scale", [in_channels],
                                initializer=tf.constant_initializer(1.0))
        offset = tf.get_variable("beta", [in_channels],
        ...
        mean, variance = tf.nn.moments(features, [0, 1, 2], keep_dims=True)
        out = tf.nn.batch_normalization(features, mean, variance, scale, offset, variance_epsilon=1e-4)
        return tf.nn.relu(upconv(out, in_channels, out_channels))
...
def concat_bn_conv_relu_2_bn_upconv_relu(features_down, features_up, name='concat_bn_conv_relu_2_bn_upconv_relu'):
    with tf.name_scope(name):
        ...
        return bn_upconv_relu(out, 64, 64)
...
def conv_net(features):
    ...
    features_down3_skip_conn, features_down3 = bn_conv_relu_3_maxpool(features_down2, 64, name='down3')
    features_down4_skip_conn, features_down4 = bn_conv_relu_3_maxpool(features_down3, 64, name='down4')
    features_down5_skip_conn, features_down5 = bn_conv_relu_3_maxpool(features_down4, 64, name='down5')
    features_down6_skip_conn, features_down6 = bn_conv_relu_3_maxpool(features_down5, 64, name='down6')

    with tf.name_scope('up1'):
        features_up1 = bn_conv_relu(features_down6, 64, 64)
        features_up1 = bn_conv_relu(features_up1, 64, 64)
        features_up1 = bn_upconv_relu(features_up1, 64, 64)

    features_up2 = concat_bn_conv_relu_2_bn_upconv_relu(features_down6_skip_conn, features_up1, name='up2')
    features_up3 = concat_bn_conv_relu_2_bn_upconv_relu(features_down5_skip_conn, features_up2, name='up3')
    features_up4 = concat_bn_conv_relu_2_bn_upconv_relu(features_down4_skip_conn, features_up3, name='up4')
    features_up5 = concat_bn_conv_relu_2_bn_upconv_relu(features_down3_skip_conn, features_up4, name='up5')
